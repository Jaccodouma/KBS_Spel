...
void GameColour::generateNewColour() {
	...
	rgb newColourRGB =huetoRGB(random(0,360));
	this->setGameColour(newColourRGB.r,newColourRGB.g,newColourRGB.b);
}

rgb GameColour::huetoRGB(uint16_t hue) {
	rgb out; 
	uint8_t segment;
	uint8_t slope;
...
// Copied from the adafruit library so that we don't need a screen object in here (:
